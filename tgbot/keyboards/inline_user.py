...
def start_menu_finl(user_id: Optional[int] = None) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardBuilder()
    ...
    if user_id is not None and user_id in get_admins():
        keyboard.row(ikb("Админ панель", data="admin_panel"))
...
def refill_bill_finl(
    ...
    keyboard.row(ikb("🌀 Перейти к оплате", url=pay_link))
    keyboard.row(ikb("🔄 Проверить оплату", data=f"Pay:{pay_method}:{pay_receipt}"))
