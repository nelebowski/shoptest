...
async def buy_currency_account(message: Message, bot: Bot, state: FSM, arSession: ARS):
    ...
    amount = data["buy_amount"]  # млн

    account = message.text
    pay_amount = amount * PRICE_PER_MILLION_RUB  # ₽
    stars_amount = math.ceil(pay_amount / STARS_RATE_RUB_PER_STAR)
...
async def order_pay_check(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    if pay_method == "Yoomoney":
        pay_status, _ = await YoomoneyAPI(bot=bot, arSession=arSession, update=call).bill_check(pay_receipt)
    else:
        pay_status, _ = await CryptobotAPI(bot=bot, arSession=arSession, update=call).bill_check(pay_receipt)
...
async def order_stars_success(message: Message, bot: Bot, state: FSM, arSession: ARS):
    payload = message.successful_payment.invoice_payload
    if not payload.startswith("buy:"):
        return

    # payload: buy:{receipt}:{server}:{account}:{amount}:{pay_amount}
    parts = payload.split(":", 5)
    if len(parts) != 6:
        return

    _, pay_receipt, server, account, amount, pay_amount = parts
    try:
...
async def user_available_swipe(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    if not items_available:
        return await call.answer("Список пуст", True)
